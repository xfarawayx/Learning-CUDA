...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
  ...
  // Get device properties for warp size
  cudaDeviceProp prop;
  cudaGetDeviceProperties(&prop, 0);

  const int n = std::min(rows, cols);
  const int block = BLOCK_SIZE, grid = (n + block - 1) / block;
  const int smem_size = prop.warpSize * sizeof(T);

  T* d_input = nullptr, *d_output = nullptr;
  
  cudaMalloc((void **)&d_input, sizeof(T) * rows * cols);
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
  ...
  cudaMalloc((void **)&d_o, sizeof(T) * tgt_size);

  cudaMemcpy(d_q, h_q.data(), sizeof(T) * tgt_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_k, h_k.data(), sizeof(T) * src_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_v, h_v.data(), sizeof(T) * src_size, cudaMemcpyHostToDevice);

  flashAttention_kernel<<<grid, block, smem_size>>>(d_q, d_k, d_v, d_o, target_seq_len, src_seq_len, query_heads, kv_heads,
